...
# Breaking: disable ypbind on server
import pexpect
import sys
...
if __name__ == "__main__":
    ...
    ssh_client = establish_ssh_and_setup("127.0.0.1", "check_NIS_config.py")
    debug("DONE\n")

    # Establish ssh to server and run tests
    debug("Running Tests for server")
    ssh_client = establish_ssh_and_setup("10.0.0.2", "check_NIS_config.py", ssh_client)
    debug("     Running test: \"check_NIS_config.py\"")
    ssh_client.sendline("python3 check_NIS_config.py ")
    ssh_client.expect("PASSED" or pexpect.TIMEOUT)
    debug("PASSED" + "\n")

    # Cleanup on server and exit back to router
    ssh_client.sendline("rm check_NIS_config.py")
    ...
    ssh_client = establish_ssh_and_setup("10.0.0.3", "check_NIS_config.py", ssh_client)
    debug("     Running test: \"check_NIS_config.py\"")
    ssh_client.sendline("python3 check_NIS_config.py ")
    ssh_client.expect("PASSED" or pexpect.TIMEOUT)
    debug("PASSED" + "\n")
    ...
    ssh_client = establish_ssh_and_setup("10.0.0.4", "check_NIS_config.py", ssh_client)
    debug("     Running test: \"check_NIS_config.py\"")
    ssh_client.sendline("python3 check_NIS_config.py ")
    ssh_client.expect("PASSED" or pexpect.TIMEOUT)
    debug("PASSED" + "\n")
